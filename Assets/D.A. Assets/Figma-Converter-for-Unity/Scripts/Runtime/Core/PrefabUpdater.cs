...
{
    [Serializable]
    public class PrefabUpdater : MonoBehaviourLinkerRuntime<FigmaConverterUnity>
    {
        public void UpdatePrefabs()
        {
            CancellationTokenSource cts = monoBeh.CancellationTokenController.CreateNew(TokenType.Import);
            _ = UpdatePrefabsAsync(cts);
        }

        private async Task UpdatePrefabsAsync(CancellationTokenSource cts)
        {
#if UNITY_EDITOR
