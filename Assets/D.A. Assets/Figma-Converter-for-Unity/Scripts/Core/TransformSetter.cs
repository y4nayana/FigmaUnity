...
namespace DA_Assets.FCU
{
    ...
    public class TransformSetter : MonoBehaviourBinder<FigmaConverterUnity>
    {
        ...
        public Rect GetRotatedRect(FObject fobject)
        {
            ...
            monoBeh.Log($"{nameof(GetRotatedRect)} | {fobject.Data.NameHierarchy} | {size} | {position}", FcuLogType.Transform);
        ...
        public Rect GetGlobalRect(FObject fobject)
        {
            ...
            float scale = 1;
            bool hasScaleInName = fobject.Data.FcuImageType == FcuImageType.Downloadable && fobject.Data.SpritePath.TryParseSpriteName(out scale, out var _);

            int state = 0;

            if (hasScaleInName && rPos != bPos)
            {
                if (hasRenderPos)
                {
                    state = 1;
        ...
        public IEnumerator SetNovaPositions(List<FObject> fobjects)
        {
            ...
            foreach (FObject fobject in fobjects)
            {
                ...
                {
                    fobject.Data.GameObject.TryAddComponent(out Nova.TextBlock tb);
                }
                else
                {
                    fobject.Data.GameObject.TryAddComponent(out Nova.UIBlock2D uib2d);
                }

                Nova.UIBlock uiBlock = fobject.Data.GameObject.GetComponent<Nova.UIBlock>();

                uiBlock.Layout.Size = new Nova.Length3
                {
